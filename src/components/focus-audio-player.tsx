  ...
  if (!preview && !store.entitlements?.features.music)
    return (
      <aside className={styles.player} aria-label="Ambiances audio">
        <Headphones size={18} /><span>Ambiances pour se concentrer</span>
        <Button variant="ghost" onClick={() => navigate('billing')}>Découvrir Pro</Button>
      </aside>
    );
...
function Player({ preview }: { preview: boolean }) {
    ...
    const timer = setTimeout(() => {
      try {
        ...
        setPreferences({
          ...
          pauseOnBreak: typeof saved.pauseOnBreak === 'boolean' ? saved.pauseOnBreak : defaults.pauseOnBreak,
        });
      } catch { /* Private browsing can make optional device preferences unavailable. */ }
    }, 0);
    return () => {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/audio?workspaceId=${encodeURIComponent(store.workspaceId)}`, { cache: 'no-store', signal: controller.signal })
      .then(async (response) => {
      ...
      .catch((cause) => {
        if (!controller.signal.aborted) setError(cause instanceof Error ? cause.message : 'Le catalogue audio est indisponible.');
      })
  ...
  function save(next: Partial<Preferences>) {
    ...
    try { localStorage.setItem(storageKey, JSON.stringify(value)); } catch { /* Optional local preferences. */ }
  }
  function stop() {
    version.current++;
    audioRef.current?.pause();
  ...
  async function play() {
    ...
    try {
      if (preview) {
        ...
        const response = await fetch(`/api/audio?workspaceId=${encodeURIComponent(store.workspaceId)}&trackId=${encodeURIComponent(selected.id)}`, { cache: 'no-store', signal: AbortSignal.timeout(10_000) });
        const data = await response.json();
      ...
      if (current === version.current) {
        ...
        if (blobRef.current) URL.revokeObjectURL(blobRef.current);
        blobRef.current = '';
        setError(cause instanceof Error && cause.name !== 'NotAllowedError' ? cause.message : 'Le navigateur a bloqué la lecture. Cliquez à nouveau sur Écouter.');
      }
    } finally {
    ...
    <details className={styles.player}>
      ...
      <div className={styles.controls}>
        ...
        {loading ? <p className="helper">Chargement des ambiances…</p> : !tracks.length ? <p className="helper">Aucune piste n’est encore disponible dans le catalogue.</p> : <>
          <label>Ambiance
            <select value={selected?.id || ''} disabled={busy} onChange={(event) => { stop(); save({ trackId: event.target.value }); setError(''); }}>
              {tracks.map((track) => <option key={track.id} value={track.id}>{track.title}</option>)}
            </select>
          </label>
          <div className={styles.transport}>
            <Button variant="secondary" disabled={busy || (!playing && breakPaused)} onClick={() => playing ? audioRef.current?.pause() : void play()}>
              {busy ? <RefreshCw size={16} /> : playing ? <Pause size={16} /> : <Play size={16} />}{busy ? 'Préparation…' : playing ? 'Mettre en pause' : 'Écouter'}
            </Button>
            <label>Volume : {Math.round(preferences.volume * 100)} %
              <input type="range" min="0" max="1" step="0.05" aria-label="Volume des ambiances" value={preferences.volume} onChange={(event) => save({ volume: Number(event.target.value) })} />
            </label>
          </div>
          <div className={styles.options}>
            <label><input type="checkbox" checked={preferences.loop} onChange={(event) => save({ loop: event.target.checked })} />En boucle</label>
            <label><input type="checkbox" checked={preferences.pauseOnBreak} onChange={(event) => save({ pauseOnBreak: event.target.checked })} />Pause pendant les pauses du minuteur</label>
          </div>
          {breakPaused && <p className="helper">Le minuteur est en pause courte ou longue. La lecture restera arrêtée jusqu’à votre prochain clic sur Écouter.</p>}
