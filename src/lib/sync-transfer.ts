...
const entitySchemas = {
  ...
  noteSheets: noteSheetSchema,
  flashcardDecks: flashcardDeckSchema,
  flashcards: flashcardSchema,
  events: activityEventSchema,
};
const entity = z.object({ id: z.string().uuid() }).passthrough();
const collectionPatch = z
  .object({
  ...
  .superRefine((change, context) => {
    for (const values of [
      change.remove,
      change.replace.map((item) => item.id),
      change.insert.map((item) => item.value.id),
      change.insert.map((item) => item.at),
    ]) {
      if (new Set<string | number>(values).size !== values.length)
        context.addIssue({
          code: 'custom',
          message: 'La modification contient des identifiants ou positions en double.',
        });
    }
  });
export const documentPatchSchema = z
  ...
  .strict()
  .transform((patch) => {
    // Send the same trimmed/defaulted records to SQL that domain validation sees.
    for (const key of syncCollections) {
      const change = patch.collections[key];
      if (!change) continue;
      change.replace = change.replace.map((record) => entitySchemas[key].parse(record));
      change.insert = change.insert.map((entry) => ({
        ...entry,
        value: entitySchemas[key].parse(entry.value),
      }));
    }
    return patch;
  });
...
export function makeDocumentPatch(previous: WorkspaceData, next: WorkspaceData): DocumentPatch {
  if (previous.workspaceId !== next.workspaceId)
    throw new Error('Les espaces à synchroniser ne correspondent pas.');
  const collections: DocumentPatch['collections'] = {};
  for (const key of syncCollections) {
    const before = previous[key] as RecordData[],
      after = next[key] as RecordData[];
    ...
    const replace = after.filter(
      (item) =>
        original.has(item.id) && JSON.stringify(original.get(item.id)) !== JSON.stringify(item),
    );
    ...
    if (remove.length || replace.length || insert.length || reordered)
      collections[key] = {
        ...
        replace,
        insert,
        ...(reordered ? { order: after.map((item) => item.id) } : {}),
      };
  }
...
export function applyDocumentPatch(previous: WorkspaceData, input: unknown): WorkspaceData {
  ...
  if (patch.metadata.workspaceId !== previous.workspaceId)
    throw new Error('Le document appartient à un autre espace.');
  ...
  for (const key of syncCollections) {
    ...
    let items = (previous[key] as RecordData[])
      .filter((item) => !removed.has(item.id))
      .map((item) => replacements.get(item.id) || item);
    for (const entry of [...change.insert].sort((a, b) => a.at - b.at))
      items.splice(entry.at, 0, entry.value);
    if (change.order) {
      const values = new Map(items.map((item) => [item.id, item]));
      if (
        change.order.length !== items.length ||
        new Set(change.order).size !== items.length ||
        change.order.some((key) => !values.has(key))
      )
...
export function snapshotPage(data: WorkspaceData, cursor = 0, maxBytes = 450_000) {
  ...
  if (!Number.isSafeInteger(cursor) || cursor < 0 || cursor > total)
    throw new Error('Page de synchronisation invalide.');
  ...
  return {
    metadata: cursor === 0 ? documentMetadata(data) : undefined,
    entries,
    cursor,
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    while (true) {
      ...
      if (
        ...
        (version !== undefined && version !== body.version)
      )
        throw new Error('La réponse de synchronisation est incohérente.');
      version = body.version;
      if (cursor === 0) {
        result = { ...result, ...body.metadata };
        expectedTotal = body.total;
      }
      if (body.total !== expectedTotal)
        throw new Error('Le nombre d’éléments a changé pendant le chargement.');
      for (const entry of body.entries) {
        if (!syncCollections.includes(entry.collection))
          throw new Error('Collection de synchronisation inconnue.');
      ...
      if (body.nextCursor !== next || next <= cursor)
        throw new Error('Page de synchronisation invalide.');
      cursor = next;
    }
  }
