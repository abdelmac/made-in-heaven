...
test('an overlapping drop keeps the original session and reports the conflict', async ({
  ...
  await expect(page.locator('.full-planner').getByRole('alert')).toContainText(plannerCopy.overlap);
  expect((await readData(page)).plannedSessions[0]).toEqual(original);
  await page.keyboard.press('Escape');
  await expect(
    page.getByRole('button', { name: 'Annuler le déplacement', exact: true }),
  ).toHaveCount(0);
});
...
test('tasks can be dragged to the grid and sessions can also move using the keyboard', async ({
  ...
  await page
    .getByRole('button', { name: `${plannerCopy.move} ${task.title}`, exact: true })
    .focus();
