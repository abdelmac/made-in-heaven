# Billing environments and launch safety
...
Solace uses Stripe-hosted Checkout and the Customer Portal. The prepared version defaults to test mode and rejects any key or resource that belongs to the other mode. Live mode requires an explicit Stripe account, a matching database binding (migration `0011`), complete seller information and separate launch attestations before new checkout is allowed. No card details enter Solace. Without credentials, pricing remains unavailable and the local/demo application stays usable. Opening a successful checkout return URL never grants access. Follow the current [launch runbook](launch-runbook.md): migration `0011` is now applied with a legacy-compatible `test/NULL` binding, but the new application code is not yet deployed and Stripe remains unconfigured.

**Credential incident:** the private live key previously checked below was subsequently found in tracked `.env.example`. The current file is cleaned, but Git history is not. Rotate/revoke that key before any further provider use or launch. No key value belongs in documentation or chat. `npm run security:secrets` now guards the working project files and CI.
...
The replacement restricted live key now passes the read-only account check: Stripe reports `charges_enabled: true`, `payouts_enabled: true`, `details_submitted: true`, card payments `active`, and no currently/past-due account requirements. Account, product, selected-price, portal-configuration and webhook-endpoint reads succeed. No resources tagged for this new installation were found; write permissions and the actual purchase lifecycle remain unverified. The account has no configured support e-mail. The earlier HTTP 401/403 checks are resolved. The Vercel team API still reports `hobby`. No live resource, subscription, payment, or deployment was created. Keep the working key private; do not commit it or paste it into chat. Other launch prerequisites remain:
...
- Deploy the prepared explicit test/live separation in server validation, persisted billing state, SQL entitlements and UI notices after local verification and applying migration `0011` to the correct target. Never reuse the old site's test subscriptions as live access.
...
## Historical September 16 sandbox offering — separate old installation
...
Real payments require an activated Stripe account and a deliberately enabled, reviewed live-mode deployment. Replacing the key alone is insufficient: `STRIPE_BILLING_MODE`, `STRIPE_ACCOUNT_ID` and the database binding must match, every provider resource must belong to that mode, and the live launch gate must be explicitly open. The gate closes new checkout only; existing invoices, cancellation and signed webhook reconciliation remain available. See the [launch runbook](launch-runbook.md) for the required attestations, migration order, seller details and safe shutdown procedure. [Stripe go-live checklist](https://docs.stripe.com/get-started/checklist/go-live)
...
2. In a Stripe sandbox/test account, create the current **Solace Pro** Prices at **EUR 3.99/month** and **EUR 29.99/year**, and fill `STRIPE_PRO_MONTH_PRICE_ID` and `STRIPE_PRO_YEAR_PRICE_ID` in `.env.local`. Use licensed, flat, per-unit billing, quantity one. Leave Team IDs empty for this launch. The historical EUR 1 sandbox offer above is not the new offer. Unset intervals are unavailable; there are no fallback prices. Amounts, currency and intervals are retrieved from Stripe using its official SDK.
...
Signatures are verified over the exact raw request body using Stripe's SDK and default timestamp tolerance. Only the configured mode is accepted; Connect events are rejected. Verified event IDs and their billing mode are inserted durably before processing after checking the database/account binding. Processed duplicates return successfully without reapplying state. Failed processing returns HTTP 503, leaving a retryable receipt; Stripe retries or an operator can resend the event. Unrelated event types/customer associations are durably acknowledged without altering workspace access.
