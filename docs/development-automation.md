...
Le code du workflow est prêt ; ajouter ces fichiers ne configure pas les comptes externes. Le job de déploiement reste désactivé tant que les variables de dépôt `VERCEL_DEPLOY_ENABLED` et `SOLACE_BILLING_MIGRATION_0011_READY` ne sont pas toutes deux exactement `true`. Ce deuxième verrou évite de publier automatiquement le nouveau serveur avant la préparation de sa base.
...
6. Après avoir réellement appliqué et vérifié `0011` sur la base cible, définir la variable **de dépôt** `SOLACE_BILLING_MIGRATION_0011_READY=true`. C’est une attestation opérateur, pas une migration automatique ni une vérification distante par la CI. Puis définir `VERCEL_DEPLOY_ENABLED=true`. Les prochains push sur `main`, ou un lancement manuel du workflow sur `main`, déploieront seulement après les vérifications réussies. Les PR ne déploient jamais et ne reçoivent pas les secrets Vercel.
