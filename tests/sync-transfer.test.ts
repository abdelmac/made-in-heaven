...
import { applyDocumentPatch, fetchWorkspaceSnapshot, makeDocumentPatch, snapshotPage } from '@/lib/sync-transfer';
...
describe('incremental document transfer', () => {
  it('preserves exact collection ordering across edits, deletion, insertion and reordering', () => {
    const previous = createDemoData(), next = structuredClone(previous);
  ...
  it('does not retransmit unchanged large histories', () => {
    const previous = createDemoData(), next = structuredClone(previous);
    previous.noteSheets = Array.from({ length: 80 }, () => ({ id: id(), workspaceId: previous.workspaceId, userId: '00000000-0000-4000-8000-000000000002', createdAt: previous.updatedAt, updatedAt: previous.updatedAt, title: 'Note', content: 'a'.repeat(15000), kind: 'note' as const, revisions: [] }));
  ...
  it('rejects foreign workspaces, invalid collections and broken links without changing the source', () => {
    const data = createDemoData(), patch = makeDocumentPatch(data, data);
    ...
    expect(() => applyDocumentPatch(data, { ...invalid, collections: { memberships: {} } })).toThrow();
  });
});
...
describe('paged consistent snapshots', () => {
  it('reassembles pages only after receiving the whole version', async () => {
    ...
    const fetcher = vi.fn(async (input: string | URL | Request) => {
      ...
      return Response.json({ ...snapshotPage(data, Number(params.get('cursor')), 600), version: 8 });
    });
  ...
  it('discards partial data on concurrent changes and starts from the next complete version', async () => {
    ...
    const fetcher = vi.fn(async (input: string | URL | Request) => {
      ...
      if (cursor > 0 && !conflicted) { conflicted = true; return Response.json({}, { status: 409 }); }
  ...
  it('handles an empty workspace and rejects truncated page sequences', async () => {
    ...
    const fetcher = vi.fn(async () => Response.json({ ...snapshotPage(data), total: 1, version: 1 }));
